...
describe('Localize', () => {
    ...
    it('External Data German flat', () => {
        let localize: any = new Translator({ locale: 'de-DE', localPath: path.join(__dirname, '..', '..', 'src', '__tests__', 'feature') });
        expect(localize.translate('json.schemas.desc', 'Guten')).toEqual('Guten Tag Welt');
    });
})